...
# April 29, 2022 - removed code info 
#

description <- function(netFile,csvFile,T){
  f <- function(L){
# extracts work's label from the line L
  ...
  nn <- length(N); 
  tit <- character(nn); aut <- character(nn); jrn <- character(nn)
  wos <- integer(nn); yer <- integer(nn) #;cod <- character(nn)
...
#   cat(k,yer[k],tit[k],'\n'); flush.console()
  ...
  D <- data.frame(name=as.character(N),WoSline=wos,author=as.character(aut),
     title=as.character(tit),journal=as.character(jrn),year=yer)
     #,code=as.character(cod))
