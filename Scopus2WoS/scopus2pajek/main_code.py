...
for i in range(len(A)):
    ID = A['IDa'][i]
    nam = A['au'][i].lower()
    if (ID == "nan") or (nam == "anon"):
        nanon += 1
        A['IDa'][i] = "anon " + str(nanon)
        nam = "anon " + str(nanon)
        A['au'][i] = nam
#
...
with open("WA.net", "w", encoding="UTF-8") as net:
    ...
    for i in range(len(U)):
        net.write(str(U[i]) + ' ' + str(V[i] + nr + 1) + ' ' + str(w[i]) + ' [' + str(t[i]) + ']\n')
...
def stand(s): # s represents author names separated by commas
    ...
    if k > 1: # checks if the list has more than one element
        ...
        p.append("")
        k = 2 # if there is only one element in the list, it appends an empty string to it, and sets k to 2ž
